...
const isImage = (type === 'imageMessage')
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
        ...
        ////DAFTAR BUTTON 
      const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu} \n\nSebelum Menggunakan VIP BOT Verify Terlebih Dahulu Ya `
      ...
      // GAME 
        ...
        if (game.isfam(from, family100)) {
          ...
          if (tebakata.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
			...
			tc = `MAAF KA NOMER KAMU BLOM TERDAFTAR DI DATABASE SILAHKAN KETIK !verify UNTUK MENDAFTAR DI VIP BOT`
			ind = {
...
*NOMOR CREATOR*
▼▼▼▼▼`)
sendKontak(from, `6287701656619`, `${ownerName}`, 'Sibukk!!')
               await sleep(1000)
               break
               case 'spamcall':{
...
🐟 ${prefix}wallgravity text1 text2
 `
sendButMessage(from, wibu, `created VIP BOT`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
        ...
        case 'wasted':
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Jangan Lupa Subscribe CLONBOT`},type:1}]
              ...
              contentText:`Jangan Lupa Subscribe CLONBOT`,buttons,headerType:4}
...
reply (mess.wait)
             buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/${command}?apikey=IkyOgiwara`)
               buttons = [{buttonId: `!memeindo`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'VIP BOT', imageMessage: imageMsg,
              contentText:`Jangan Lupa Subscribe CLONBOT`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
...
`
 ...
 VIP BOT`, imageMessage: imageMsg,
   ...
   case 'command':
               list = []
               listmenu = [`groupmenu`,`photoxy`,`makermenu`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`grup`,`sewabot`]
               listmenuu = [`Menu Group`,`Photo Oky`,`Image Maker`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`]
               nombor = 1
               startnum = 0
               ...
               listmsg(from, `${ucapanWaktu} Kak ${pushname} 🗿👋\n\n*SILAHKAN PILIH*\n\n🦈 Day : ${week} ${weton} \n🦈 Date : ${date}\n\n╭─「 ❉ *YOU INFO* ❉ 」─\n│\n│•🦈 *Username:* ${pushname}\n│•🦈 *Bio Info:* ${thu.status} \n│•🦈 *User*: ${isPremium ? 'Premium User' : 'Free User'}\n│•🦈 *Admin*: ${isGroupAdmins ? 'Ya' : 'No'}\n│•🦈 *Level*: ${Levelnye}\n│•🦈 *XP*: ${Xpluu} ${requiredXplu}\n╰───────────────`, `  `, list)
               break
               case 'tts':
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: freply})
					  try{
...
break 
               case 'grup':
               reply(`https://chat.whatsapp.com/LF76mRDRwLlI4pdbMi0d5A`)
               break
   case 'izinout':
   list = []
               listmenu = [`adagc`,`menu`]
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
     ...
     case 't':
       titid =  `SILAHKAN PILIH DI BAWAH UNTUK MELANJUTKAN`
                   ...
                   sendButMessage(from, titid, `crated VIP BOT`, [
          {
            buttonId: `Y`,
            buttonText: {
              displayText: `Y`,
...
*「 VIP BOT 」*`
	    ...
	    // console.log(e)
            setTimeout( () => {
			ikyy.updatePresence(from, Presence.composing)
			reply(`*Terimakasih Telah Mendaftar Di VIP BOT*`)
		}, 2000)
        break
case 'sfire':
{
...
if (args.length < 1) return reply('Jumlahnya?')
       ...
       case 'sc': 
       case 'src':
            teksnya = `*NGGAK ADA SC🗿*
`
  buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: `Owner👑`},type:1}]
              buttonsMessage = {footerText:'BUAT SENDIRI TOD', imageMessage: imageMsg,
              contentText: teksnya,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
              break
...
//------------------< Fun >-------------------
      ...
      break
       ...
       case 'kapankah':
              ...
              case 'truthordare':
              case 'truth or dare':
              truthordare = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
              ikyy.sendMessage(from, { contentText: `PILIH SALAH SATU`, footerText: 'Pilih salah satu', buttons: [{ buttonId: `!truth`, buttonText: { displayText: 'Truth' }, type: 1 },{ buttonId: `!dare`, buttonText: { displayText: 'Dare' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: truthordare, contextInfo: {mentionedJid: [truthordare]}}}, 'buttonsMessage')
              break
       case 'truth':
...
if (budy.includes("Numa")){
		...
		const loli = fs.readFileSync('./assets/numa.mp3')
...
if (budy.includes(`kontol`)) {
	...
	reply('./sticker/bot.webp');

        ikyy.sendMessage(from, d, sticker, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "song : Anak Kontol", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
}
if (budy.includes("halo bot")){
		ikyy.updatePresence(from, Presence.composing)

		const loli = fs.readFileSync('./assets/bot.mp3')

        ikyy.sendMessage(from, loli, MessageType.audio, {quoted: freply, mimetype: 'audio/mp4', ptt:true})

        const d = fs.readFileSync('./sticker/jget.webp');

        ikyy.sendMessage(from, d, sticker, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "song : Halo Juga", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
